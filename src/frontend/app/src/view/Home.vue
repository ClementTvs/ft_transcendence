...
async function toggleComments(postId) {
  ...
  if (expandedComments.value[postId] && !commentsData.value[postId]) {
    ...
    try { commentsData.value[postId] = await getComments(postId) }
    catch (e) { commentsData.value[postId] = [] }
  ...
  <div :class="dark ? 'bg-gray-950' : 'bg-rose-50/50'" class="min-h-[calc(100vh-64px)]">
    <div class="max-w-6xl mx-auto flex gap-5 px-4 py-5">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div :class="dark ? 'bg-gray-900 border-gray-700' : 'bg-white border-rose-100'" class="rounded-2xl border mb-5 shadow-sm overflow-hidden">
          <div class="p-5">
            <div class="flex gap-3">
              ...
              <div class="flex-1">
                <textarea
                  ...
                  rows="2"
                  :class="dark ? 'text-white placeholder-gray-500' : 'text-gray-800 placeholder-gray-400'"
                  class="w-full resize-none bg-transparent focus:outline-none text-[15px] leading-relaxed"
                  @keydown.ctrl.enter="handleCreatePost"
                />
                <div v-if="newPostGame" class="mt-2 flex items-center gap-2">
                  <span :class="getGameInfo(newPostGame).color" class="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full">
          ...
          <div :class="dark ? 'border-gray-700 bg-gray-800/50' : 'border-rose-50 bg-rose-50/30'" class="flex items-center justify-between px-5 py-3 border-t">
            <div class="relative">
              <button @click="showGamePicker = !showGamePicker" :class="dark ? 'text-gray-400 hover:text-gray-200' : 'text-gray-400 hover:text-gray-600'" class="flex items-center gap-1.5 text-sm transition-colors">
                <span class="text-base">🎮</span>
                <span class="text-xs font-medium">Tag un jeu</span>
              </button>
              <div v-if="showGamePicker" :class="dark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'" class="absolute bottom-full left-0 mb-2 w-56 rounded-xl shadow-xl border overflow-hidden z-50">
                <div class="max-h-64 overflow-y-auto py-1">
                  <button
                    v-for="game in gamesList" :key="game.name"
                    @click="selectGame(game)"
                    :class="dark ? 'text-gray-200 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-50'"
                    class="w-full text-left px-4 py-2 text-sm flex items-center gap-2 transition-colors"
                  >
                    <span>{{ game.icon }}</span><span>{{ game.name }}</span>
                  </button>
                </div>
              </div>
            </div>
            <div class="flex items-center gap-3">
              ...
              <button
                ...
                :class="newPostContent.trim() && !posting ? 'bg-gray-900 hover:bg-gray-800 text-white' : (dark ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-gray-100 text-gray-300 cursor-not-allowed')"
                class="px-5 py-1.5 rounded-full text-sm font-medium transition-all"
              >{{ posting ? 'Envoi...' : 'Publier' }}</button>
            </div>
          </div>
        </div>
        ...
        <div v-if="posts.length === 0" class="text-center py-20">
          ...
          <p :class="dark ? 'text-gray-500' : 'text-gray-400'" class="text-sm">Aucun post pour le moment</p>
        </div>

        <div
          v-for="post in posts" :key="post.id"
          :class="dark ? 'bg-gray-900 border-gray-700 hover:shadow-gray-900/40' : 'bg-white border-rose-100 hover:shadow-rose-100/40'"
          class="rounded-2xl border mb-3 overflow-hidden shadow-sm hover:shadow-md transition-all"
        >
          <div class="p-5">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                  <router-link :to="`/user/${post.author_id}`" :class="dark ? 'text-white' : 'text-gray-900'" class="font-bold text-[15px] hover:underline">
                ...
                >{{ getGameInfo(parsePost(post).game).icon }} {{ parsePost(post).game }}</span>
                ...
                <div class="flex items-center gap-8 mt-4">
                  <button @click="handleLike(post)" class="group flex items-center gap-2">
                    <div :class="post.is_liked ? 'bg-rose-50 text-rose-500' : (dark ? 'text-gray-500 group-hover:bg-rose-500/10 group-hover:text-rose-500' : 'text-gray-400 group-hover:bg-rose-50 group-hover:text-rose-500')" class="p-1.5 rounded-full transition-all">
                      <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" :fill="post.is_liked ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
                      </svg>
                    </div>
                    <span :class="post.is_liked ? 'text-rose-500' : (dark ? 'text-gray-500 group-hover:text-rose-500' : 'text-gray-400 group-hover:text-rose-500')" class="text-sm transition-colors">{{ post.like_count }}</span>
                  </button>
                  <button @click="toggleComments(post.id)" class="group flex items-center gap-2">
                    <div :class="expandedComments[post.id] ? 'bg-sky-50 text-sky-500' : (dark ? 'text-gray-500 group-hover:bg-sky-500/10 group-hover:text-sky-500' : 'text-gray-400 group-hover:bg-sky-50 group-hover:text-sky-500')" class="p-1.5 rounded-full transition-all">
                      <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
                      </svg>
                    </div>
                    <span :class="expandedComments[post.id] ? 'text-sky-500' : (dark ? 'text-gray-500 group-hover:text-sky-500' : 'text-gray-400 group-hover:text-sky-500')" class="text-sm transition-colors">{{ post.comment_count }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div v-if="expandedComments[post.id]" :class="dark ? 'border-gray-700 bg-gray-800/50' : 'border-gray-100 bg-gray-50/50'" class="border-t">
            <div v-if="loadingComments[post.id]" :class="dark ? 'text-gray-500' : 'text-gray-400'" class="px-5 py-6 text-center text-sm">Chargement...</div>
            <div v-else>
              <div v-if="commentsData[post.id]?.length === 0" :class="dark ? 'text-gray-500' : 'text-gray-400'" class="px-5 py-6 text-center text-sm">Pas encore de commentaire</div>
              <div v-for="comment in commentsData[post.id]" :key="comment.id" :class="dark ? 'border-gray-700/50' : 'border-gray-100/50'" class="px-5 py-3 flex gap-3 border-b last:border-0">
                <img :src="avatarUrl(comment.author)" class="h-8 w-8 rounded-full object-cover flex-shrink-0 mt-0.5" />
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <span :class="dark ? 'text-gray-200' : 'text-gray-800'" class="font-semibold text-sm">{{ comment.author?.display_name || comment.author?.username }}</span>
                    <span class="text-gray-400 text-xs">{{ formatDate(comment.created_at) }}</span>
                  </div>
                  <p :class="dark ? 'text-gray-300' : 'text-gray-700'" class="text-sm leading-relaxed mt-0.5">{{ comment.content }}</p>
                </div>
              </div>
            </div>
            <div :class="dark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-100'" class="px-5 py-3 flex gap-3 items-center border-t">
              ...
              <input
                ...
                :class="dark ? 'bg-gray-800 text-white placeholder-gray-500 focus:bg-gray-750' : 'bg-gray-50 text-gray-700 placeholder-gray-400 focus:bg-gray-100'"
                class="flex-1 rounded-full px-4 py-2 text-sm focus:outline-none transition-colors"
              />
              <button
                @click="handleComment(post.id)"
                :disabled="!commentTexts[post.id]?.trim()"
                :class="commentTexts[post.id]?.trim() ? 'bg-gray-900 text-white hover:bg-gray-800' : (dark ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-gray-100 text-gray-300 cursor-not-allowed')"
        ...
        <div class="sticky top-[84px] flex flex-col gap-4">
          ...
          <div :class="dark ? 'bg-gray-900 border-gray-700' : 'bg-white border-rose-100'" class="rounded-2xl border overflow-hidden shadow-sm">
            <div class="px-5 pt-4 pb-2 flex items-center justify-between">
              ...
              <span v-if="onlineFriends.length > 0" class="flex items-center gap-1.5 text-xs text-green-500">
                <span class="h-2 w-2 rounded-full bg-green-400 animate-pulse"></span>
                {{ onlineFriends.length }}
              </span>
            </div>
            <div v-if="onlineFriends.length === 0" :class="dark ? 'text-gray-500' : 'text-gray-400'" class="px-5 py-6 text-center text-sm">
              Personne en ligne pour le moment
            </div>
            <router-link
              v-for="friend in onlineFriends" :key="friend.id"
              :to="`/user/${friend.id}`"
              :class="dark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'"
            ...
            >
              <div class="relative flex-shrink-0">
                ...
                <div :class="dark ? 'border-gray-900' : 'border-white'" class="absolute -bottom-0.5 -right-0.5 h-3 w-3 bg-green-400 rounded-full border-2"></div>
              </div>
              <div class="flex-1 min-w-0">
                <p :class="dark ? 'text-white' : 'text-gray-900'" class="font-semibold text-sm truncate">{{ friend.display_name || friend.username }}</p>
            ...
            <div v-if="following.filter(f => !f.user?.is_online).length > 0" :class="dark ? 'border-gray-700' : 'border-gray-100'" class="border-t">
              ...
              <router-link
                v-for="f in following.filter(f => !f.user?.is_online).slice(0, 5)" :key="f.id"
                :to="`/user/${f.user.id}`"
                :class="dark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'"
          ...
          <div v-if="suggestions.length > 0" :class="dark ? 'bg-gray-900 border-gray-700' : 'bg-white border-rose-100'" class="rounded-2xl border overflow-hidden shadow-sm">
            <p :class="dark ? 'text-white' : 'text-gray-900'" class="px-5 pt-4 pb-2 font-bold text-[15px]">À suivre</p>
            <div v-for="s in suggestions.slice(0, 4)" :key="s.id" :class="dark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'" class="flex items-center gap-3 px-5 py-3 transition-colors">
              ...
              <div class="flex-1 min-w-0">
                <router-link :to="`/user/${s.id}`" :class="dark ? 'text-white' : 'text-gray-900'" class="font-semibold text-sm truncate block hover:underline">{{ s.display_name || s.username }}</router-link>
          ...
          <div v-if="user" :class="dark ? 'bg-gray-900 border-gray-700' : 'bg-white border-rose-100'" class="rounded-2xl border p-5 shadow-sm">
            <div class="flex items-center gap-3">
              <router-link to="/profile">
                <img :src="avatarUrl(user)" class="h-11 w-11 rounded-full object-cover" />
              </router-link>
              <div class="min-w-0">
                <p :class="dark ? 'text-white' : 'text-gray-900'" class="font-bold text-sm truncate">{{ user.display_name || user.username }}</p>
                <p class="text-xs text-gray-400 truncate">@{{ user.username }}</p>
              </div>
            </div>
            <router-link to="/profile" :class="dark ? 'border-gray-600 text-gray-300 hover:bg-gray-800' : 'border-gray-200 text-gray-600 hover:bg-gray-50'" class="block text-center mt-4 py-2 rounded-full border text-sm font-medium transition-colors">Mon profil</router-link>
          </div>
