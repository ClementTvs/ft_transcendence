...
const MAX_IMAGE_SIZE = 5 * 1024 * 1024 // 5 MB (synchro avec back)
...
function onImageSelect(e) {
  ...
  if (!allowed.includes(file.type)) {
    alert('Format non supporté (jpeg, png, gif, webp uniquement)')
    e.target.value = ''
    return
  }

  newPostImage.value = file
  newPostImagePreview.value = URL.createObjectURL(file)
}

function removeImage() {
  if (newPostImagePreview.value) URL.revokeObjectURL(newPostImagePreview.value)
...
async function handleCreatePost() {
  if ((!newPostContent.value.trim() && !newPostImage.value) || posting.value) return
  posting.value = true
  try {
    const content = newPostGame.value
      ? `[${newPostGame.value.name}] ${newPostContent.value.trim()}`
      : newPostContent.value.trim()
    await createPost(content, newPostImage.value)
    ...
    console.error(e)
    alert(e.message || 'Erreur création post')
  } finally {
    posting.value = false
  }
}
        ...
        <div :class="dark ? 'bg-gray-900 border-gray-700' : 'bg-white border-rose-100'" class="rounded-2xl border mb-5 shadow-sm overflow-visible">
          <div class="p-5">
            <div class="flex gap-3">
              ...
              <div class="flex-1">
                ...
                <!-- Image preview -->
                <div v-if="newPostImagePreview" class="mt-3 relative inline-block">
                  <img
                    :src="newPostImagePreview"
                    :class="dark ? 'border-gray-700' : 'border-rose-200'"
                    class="max-h-80 rounded-xl border"
                  />
                  <button
                    @click="removeImage"
                    type="button"
                    class="absolute top-2 right-2 bg-black/60 hover:bg-black/80 text-white rounded-full w-7 h-7 flex items-center justify-center transition-colors"
          ...
          <div :class="dark ? 'border-gray-700 bg-gray-800/50' : 'border-rose-50 bg-rose-50/30'" class="relative flex items-center justify-between px-5 py-3 border-t">
            ...
            <div class="flex items-center gap-3">
              ...
              <button @click="handleCreatePost" :disabled="(!newPostContent.trim() && !newPostImage) || posting" :class="(newPostContent.trim() || newPostImage) && !posting ? 'bg-gray-900 hover:bg-gray-800 text-white' : (dark ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-gray-100 text-gray-300 cursor-not-allowed')" class="px-5 py-1.5 rounded-full text-sm font-medium transition-all">{{ posting ? 'Envoi...' : 'Publier' }}</button>
            </div>
          </div>
        </div>
          ...
          <div class="p-5">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <img v-if="post.image_url" :src="post.image_url.startsWith('http') ? post.image_url : `${API}${post.image_url}`" class="mt-3 rounded-xl max-h-96 w-full object-cover" loading="lazy" />
